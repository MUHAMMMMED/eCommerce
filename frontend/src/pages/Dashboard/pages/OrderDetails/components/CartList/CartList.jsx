      ...
      {order && order?.items.map((item) => (
        <div className='cart_list_row' key={item?.id}>
          ...
          {item?.notes?.map((item, index) => (
            <div className='cart_list_note' key={index} style={{ display: 'block' }}>  <span className='list_note_number' style={{ display: 'block' }} > {index + 1}</span><span className='list_note_text' style={{ display: 'block' }}>{item?.note}</span> </div>))}
        </div>))}</div>
